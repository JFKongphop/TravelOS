...
function ids(changes, keyword) {
    const found = [];
    for (const c of changes) {
        const otype = (c.objectType ?? "");
        if (otype.includes(keyword))
            found.push(c.objectId ?? "");
    }
    return found;
}
...
async function main() {
    ...
    let planId = "", vaultId = "";
    {
        const r = await run("1. createTrip", () => S.createTrip(ADDR, { destination: "Tokyo", startDate: 1735689600, endDate: 1736294400, totalBudget: 2000 }));
        planId = ids(r.changes, "plan::TravelPlan")[0];
        vaultId = ids(r.changes, "vault::TravelVault")[0];
        console.log(`  Plan:  ${planId}\n  Vault: ${vaultId}`);
    }
    // 2. Deposit Funds
    await run("2. depositFunds", () => S.depositFunds({ vaultId, amount: 100000000 }) // 0.1 SUI
    );
    ...
    let reservationId = "";
    {
        const r = await run("4. bookHotel", () => S.bookHotel(ADDR, { vaultId, planId, provider: "Hilton Tokyo", amount: 100000 }));
    ...
    // 7. Cancel Booking
    if (reservationId) {
        await run("7. cancelBooking", () => S.cancelBooking({ reservationId, vaultId, amount: 100000 }));
    }
    // 8. Complete Trip
    await run("8. completeTrip", () => S.completeTrip({ vaultId }));
    // Read back final state
    console.log("\n=== Final State ===");
    const vaultObj = await CL.getObject({ id: vaultId, options: { showContent: true } });
